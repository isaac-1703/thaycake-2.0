...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            sessions = sb_get("admin_sessions",
...
def create_testimonial():
    ...
    name = (data.get('name') or 'Anônimo').strip() or 'Anônimo'
...
def upload_file():
    ...
    try:
        file_data = file.read()
        sb_upload_file("product-images", filename, file_data, content_type)
...
def admin_create_product():
    ...
    product = {
        ...
        "whatsapp_link": f"https://wa.me/5589988194690?text=Ol%C3%A1!%20Quero%20pedir%20o%20{quote(data['name'].strip(), safe='')}",
        "active": True,
        "sort_order": 0
    }

    try:
        result = sb_insert("products", product)
...
def admin_update_product(product_id):
    ...
    if data.get('name'):
        updates['name'] = data['name'].strip()[:60]
    if 'description' in data:
        updates['description'] = (data['description'] or '').strip()[:200]
    if 'price' in data:
    ...
    try:
        result = sb_update("products", updates, {"id": f"eq.{product_id}"})
        if not result:
            return jsonify({"error": "Produto não encontrado"}), 404
...
def admin_update_testimonial(testimonial_id):
    ...
    if 'comment' in data:
        updates['comment'] = (data['comment'] or '').strip()[:500]
    if 'approved' in data:
    ...
    try:
        result = sb_update("testimonials", updates, {"id": f"eq.{testimonial_id}"})
        if not result:
...
def admin_delete_testimonial(testimonial_id):
    try:
        existing = sb_get("testimonials", {"id": f"eq.{testimonial_id}"})
        if not existing:
            return jsonify({"error": "Avaliação não encontrada"}), 404
        sb_delete("testimonials", {"id": f"eq.{testimonial_id}"})
